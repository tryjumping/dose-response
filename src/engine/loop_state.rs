...
use egui::{self, epaint::image::AlphaImage, Event, RawInput};
...
pub fn build_texture_from_egui(alpha_image: AlphaImage) -> RgbaImage {
    let width = alpha_image.size[0].try_into().unwrap_or(0);
    let height = alpha_image.size[1].try_into().unwrap_or(0);
    ...
    for (index, pixel) in alpha_image.srgba_pixels(1.0).enumerate() {
...
/// Apply a (potentially) partial `ImageDelta` to an existing `AlphaImage`.
///
...
/// requested position.
pub fn egui_alpha_image_apply_delta(
    source: AlphaImage,
    pos: Option<[usize; 2]>,
    delta: AlphaImage,
) -> AlphaImage {
