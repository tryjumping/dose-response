    ...
    while let Some(mut monster_position) = monster_positions_to_process.pop_front() {
        ...
        match action {
            Action::Move(destination) => {
                ...
                let (newpos, newpath) = if monster_readonly.path.is_empty() || path_changed || !world.walkable(monster_readonly.path[0], Walkability::BlockingMonsters) {
                    // Calculate a new path or recalculate the existing one.
                    let mut path = pathfinding::Path::find(
                        pos, destination, world, Walkability::BlockingMonsters);
...
fn show_exit_stats(stats: &Stats) {
    println!("Slowest update durations: {:?}\n\nSlowest drawcall durations: {:?}",
             stats.longest_update_durations().iter().map(|dur| dur.num_microseconds().unwrap_or(i64::MAX)).map(|us| us as f32 / 1000.0).collect::<Vec<_>>(),
             stats.longest_drawcall_durations().iter().map(|dur| dur.num_microseconds().unwrap_or(i64::MAX)).map(|us| us as f32 / 1000.0).collect::<Vec<_>>());
