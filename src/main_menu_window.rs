...
impl Window {
    fn calculate_layout(&self, mouse: Mouse) -> Layout {
    use ui::Layout::*;
    ...
    let next_y = render_laid_out_text(&lines, rect, metrics, drawcalls);
    ...
    pub fn render(&self, state, drawcalls) {
        ...
        if let Some(rect) layout.highlighted_option_rect {
            if rect.contains(mouse.pos) {
                drawcalls.push(Draw::Rectangle(rect.top_left(), rect.dimensions(), colors::red));
            }
        }

        drawcalls.push(Draw::Rectangle(rect_under_mouse, colors::red));
    }

    pub fn hovered(mouse) -> Option<Options> {
